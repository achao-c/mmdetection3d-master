...
class SparseEncoder_rv(nn.Module):
    ...
    def __init__(self,
        ...
        self.rv_conv_out0 = make_sparse_convmodule(
            encoder_out_channels,
            self.output_channels_rv,
            kernel_size=(1, 1, 3),
            stride=(1, 1, 2),
            norm_cfg=norm_cfg,
            padding=(0, 0, 1),
            indice_key='None1',
            conv_type='SparseConv3d')
        self.rv_conv_out1 = make_sparse_convmodule(
            self.output_channels_rv,
            self.output_channels_rv,
            kernel_size=(1, 1, 3),
            stride=(1, 1, 2),
            ...
            indice_key='None2',
            conv_type='SparseConv3d')
        self.rv_conv_out2 = make_sparse_convmodule(
            self.output_channels_rv,
            self.output_channels_rv,
            kernel_size=(1, 1, 3),
            stride=(1, 1, 2),
            ...
            indice_key='None3',
            conv_type='SparseConv3d')
        self.rv_conv_out = nn.Sequential(self.rv_conv_out0, self.rv_conv_out1, self.rv_conv_out2)
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        # for detection head
        # bev:[200, 176, 5] -> [200, 176, 2]
        out_bev = self.conv_out(encode_features[-1])
        fea_bev = out_bev.dense()

        N, C, D, H, W = fea_bev.shape
        fea_bev = fea_bev.view(N, C * D, H, W)

        # rv:[200, 176, 5] -> [200, 22, 5]
        out_rv = self.rv_conv_out(encode_features[-1])
        fea_rv = out_rv.dense()
        N, C, D, H, W = fea_rv.shape
        fea_rv = fea_rv.permute(0, 1, 4, 3, 2)
        fea_rv = fea_rv.contiguous().view(N, C * W, H, D)
        return fea_bev, fea_rv
...
@MIDDLE_ENCODERS.register_module()
class SparseEncoderSASSD_rv(SparseEncoder_rv):
    ...
    def __init__(self,
                 ...
                 encoder_paddings=((1, ), (1, 1, 1), (1, 1, 1), ((0, 1, 1), 1,
                                                                 1)),
                 block_type='conv_module'):
        super(SparseEncoder_rv, self).__init__(
